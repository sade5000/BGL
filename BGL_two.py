...
def when_is_clicked(event):
	...
	if(btn_add_combo_value == 'Products'):#products loop
		...
		def no_of_prop():
			...
			if(int(action_no_of_prop) > 0):
				...
				for x in range(int(action_no_of_prop)):#enter no of prop
					entry_prop_define = Entry(btn_frame_product,font=('Roboto',10,'bold'),fg= 'green',bg= 'white')
					entry_prop_define.grid(row= 10+x , column= 0)
					entry_prop_date = Entry(btn_frame_product,font=('Roboto',10,'bold'),fg= 'green',bg= 'white')
					entry_prop_date.grid( row= 10+x, column= 1)
					print("working")
		...
		label_product_prop = Label(btn_frame_product,text= "Enter no. of Properties: ",font=('Roboto',10,'bold'),fg= 'green',bg='white')
		label_product_prop.grid(row= 9, column= 0,columnspan = 1)
		entry_no_of_prop = Entry(btn_frame_product, text="prop",font=('Roboto',10,'bold'),fg= 'green',bg='white') 
		entry_no_of_prop.grid(row= 9, column= 1)
		
		btn_add_prop = Button(btn_frame_product,text= "Add +",font=('Roboto',10,'bold'),fg= 'green',bg='white',command= no_of_prop)
		btn_add_prop.grid(row= 9 , column= 2,sticky = E)
	elif(btn_add_combo_value == 'Companies'):
		btn_frame_product.grid_forget()
		
		
	if(btn_add_combo_value == 'Products'):
